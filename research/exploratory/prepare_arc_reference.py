"""Prepare the fixed second G14 reference arc; never parse target states."""
import argparse
...
def prepare(work, products, celestial_raw, output):
    ...
    auxiliary = validate_auxiliary()
    plan_raw = (BASE/'arc_reference_plan.json').read_bytes()
    ...
    admitted, _, _, _ = frame.verified.load_inputs(frame.verified.paths('g14')['admission_receipt'].parent)
    ...
    for name in plan['stations']:
        filename = name+'_R_20262460000_01D_30S_MO.crx.gz'
        old_receipt = strict_json((frame.verified.paths('g14')['admission_receipt'].parent/'raw_observations'/(filename+'.json')).read_bytes())
        path = work/filename
        if not path.exists():
    ...
    timed = strict_json((BASE/'inputs/timed_reference_products/g14/receipt.json').read_bytes())
    for kind in ('orbit', 'clock'):
        source = timed[kind]
        raw = pinned(products/source['source_url'].split('/')[-1], source['source_compressed_sha256'])
        extract = ((BASE/'inputs/timed_reference_products/g14/reference_orbit.txt').read_bytes() if kind == 'orbit' else
    ...
    att = strict_json((BASE/'inputs/reference_attitudes/g14/receipt.json').read_bytes())
    ...
    eph.close()
    _, blq, _, _ = __import__('research.exploratory.ocean_pole_study', fromlist=['read_loading']).read_loading('g14')
    exe, build = ocean.build(work/'hardisp-build')
    terrestrial, _ = frame.read_frame()
    prior = strict_json(pinned(frame.verified.paths('g14')['station_report'], frame.verified.PINS['g14']['station_report']))
    positions = {}
    ...
    receipt = {'schema':'arc-reference-input-v1', 'auxiliary_provenance':auxiliary, 'plan_sha256':sha(plan_raw), 'sources_sha256':deps,
