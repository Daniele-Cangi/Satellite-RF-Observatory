"""Same selected products; preserve Latin-1 bibliographic metadata before extraction."""
import argparse
import gzip
...
RETAINED = {'orbit':'ffdf5305d45ed22a38cd1acd7bebf5c0e8e991dbb01aff5606c0ccef25043056',
            ...
            'bias':'5b56f40ad8c0782f3fc69c4af3d7aca9ab397cd3ebf2bed4c390f4f11c93018b'}
...
def prepare(work,output):
    ...
    receipt = {'schema':'final-reference-inputs-v3','plan_sha256':PLAN_SHA,
        ...
        try:
            ...
            if kind in RETAINED:
                ...
                acquisition = {'url':url,'sha256':first.sha(raw),'bytes':len(raw),
                               'origin':'retained v1/v2 download; local verification, not a new HTTP request',
            ...
            decoded = decoded_raw.decode('latin-1' if kind == 'bias' else 'ascii')
            receipt['products'][kind]['decoded_sha256'] = first.sha(decoded_raw)
            if kind == 'orbit':
