"""Fixed second-arc reference contrasts: local fit versus early-arc transfer."""
import argparse
...
from .hour_reference_v2 import source_bytes, COMPATIBILITY_SHA
from .hour_reference_checked import validate_auxiliary

BASE = Path(__file__).parent
INPUTS = BASE/'inputs/arc_reference'
RECEIPT_SHA = '03ba35e0b1c17458754654554a360933726b0a43103aef682ff74b918c4583fd'

def inputs():
    receipt = strict_json(pinned(INPUTS/'receipt.json', RECEIPT_SHA))
    if validate_auxiliary() != receipt['auxiliary_provenance']:
        raise ValueError('auxiliary provenance differs')
    ...
    plan = strict_json(pinned(BASE/'arc_reference_plan.json', receipt['plan_sha256']))
...
def transfer_training(plan):
    prior = strict_json(pinned(BASE/plan['transfer_training_report'], plan['transfer_training_report_sha256']))
    for key in ('date_gpst', 'target_excluded', 'stations', 'references', 'models'):
        if prior['plan'][key] != plan[key]:
            raise ValueError('transfer cohort differs: '+key)
    if not prior['calibration_complete']:
        raise ValueError('incomplete transfer training calibration')
    return [r for r in prior['rows'] if r['time_s'] < plan['transfer_training_before_gpst_s']]


def compare_models(rows, plan, prior_training):
    ...
    if not training or not testing or not prior_training:
        raise ValueError('empty chronological cohort')
    if max(r['time_s'] for r in prior_training) >= min(r['time_s'] for r in training):
        raise ValueError('transfer training must precede the second arc')
    return ([predict_block(training, testing, m) for m in plan['models']],
            [predict_block(prior_training, testing, m) for m in plan['models']])


def run(progress=None):
    plan, ctx, observations, positions, receipt = inputs()
    prior_training = transfer_training(plan)
    ...
    training = [r for r in rows if r['time_s'] < plan['training_before_gpst_s']]
    testing = [r for r in rows if r['time_s'] >= plan['training_before_gpst_s']]
    predictions, transferred = compare_models(rows, plan, prior_training) if complete else ([], [])
    return {'schema':'arc-reference-transfer-v1','source_compatibility_sha256':COMPATIBILITY_SHA,'plan':plan,'receipt_sha256':RECEIPT_SHA,
            ...
            'predictions':predictions,'transferred_predictions':transferred,
            'transfer_training_times':sorted({r['time_s'] for r in prior_training}),
            'transfer_training_report_sha256':plan['transfer_training_report_sha256'],'description':describe(rows,plan['stations'],plan['references']) if complete else None,
