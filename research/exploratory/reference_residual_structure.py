"""Time-block prediction of reference residual contrasts, not target correction."""
import argparse
...
def prepare(archive, timed, biases, antennas, attitudes, prior_report):
    ...
    for name in names:
        ...
        for obs, epoch in zip(observations, cal['epochs']):
            ...
            previous = dict(zip(epoch['references'], epoch['residuals_m']))
...
def run(archive, timed, biases, antennas, attitudes, prior_report):
    ...
    return {'schema': 'reference-residual-structure-v1', 'target_excluded': context.target, 'date_gpst': context.date_gpst,
