"""Distinct-day chronological reference contrasts; fixed models, no target fit."""
import argparse
...
INPUTS = BASE/'inputs/day_reference'
RECEIPT_SHA = '42b8c63c6799fe24428a0a0dfd492ac9d30a3a562e80fb35b8eada62819eaaf5'
...
def inputs():
    ...
    plan = strict_json(pinned(BASE/'day_reference_plan.json', receipt['plan_sha256']))
    if load_basis(plan)[-1] != receipt['auxiliary_provenance']:
...
def run(progress=None):
    ...
    precise, corrections, hashes = frame.guarded_products(ctx, plan['references'], INPUTS/'timed',
                        BASE/f'inputs/reference_biases/{tag}', BASE/f'inputs/reference_antennas/{tag}')
    ...
    return {'schema':'day-reference-prediction-v1','source_compatibility_sha256':COMPATIBILITY_SHA,'plan':plan,'receipt_sha256':RECEIPT_SHA,
