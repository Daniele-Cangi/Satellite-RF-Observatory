...
def inputs(plan_sha):
    plan = strict_json(pinned(BASE/'position_error_plan_v2.json',plan_sha))
    ...
    return plan,names,training,testing,blocks,np.array([positions[s][plan['station_epoch_index']] for s in names])
...
def error_covariances(mapping,training,testing,blocks,origin):
    ...
    for i,row in enumerate(raw):
        ...
        if i>=m:
            station[i,j]=1
            ramp[i,j]=(row['time_s']-origin)/300
...
def geometry(stations,radius,speed,plan):
    ...
    tags = np.array(plan['evaluation_times_gpst_s'])-plan['origin_gpst_s']
    def model(q):
...
def run(plan_sha,progress=None):
    ...
    for model in plan['models']:
        ...
        operators[model] = (error_covariances(mapping,train,test,blocks,plan['origin_gpst_s']),meta)
    cases = []
    ...
    return {'schema':'position-error-transfer-v2','plan':plan,'plan_sha256':plan_sha,
