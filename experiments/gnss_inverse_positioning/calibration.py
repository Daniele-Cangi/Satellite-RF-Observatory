...
# Reuse the audited RINEX-3 record parser only, not the old forward executor.
from experiments.orbital_discriminability.gnss_double_difference_screen import parse_gps_record
...
def strip_target_navigation(content: str) -> str:
    """Ignore G08 blocks as text; no target numbers are parsed or returned."""
...
def parse_reference_navigation(content: str) -> dict:
    lines = content.splitlines()
    records = {}
...
def state_and_clock(record, t):
    if record.satellite == TARGET:
        raise ValueError("target propagation is forbidden")
    # t is small seconds relative to GPST Sunday 2026-09-06; no absolute float epoch.
    tk = ((t - record.toe_sow + 302400) % 604800) - 302400
    ...
    dt = t - (record.toc_gps - DAY).total_seconds()
...
def observation_window(content: str, header, gps_types, include_target: bool):
    ...
    for row in lines:
        ...
        for record in block:
            sv = record[:3]
            if not sv.startswith("G") or (sv == TARGET and not include_target):
                continue
    ...
    if [o["time_s"] for o in observations] != list(range(0, 301, 30)):
        raise ValueError("window differs from frozen selection")
    return observations


def reference_model(record, p, t_tag, station, receiver_clock_m):
    emitted = t_tag - p / C
    t_tx = emitted
    for _ in range(4):
        _, clock = state_and_clock(record, t_tx)
        t_tx = emitted - clock
    sat, clock = state_and_clock(record, t_tx)
...
def calibrate_station(observations, station, navigation):
    epochs = []
    failures = []
    for obs in observations:
        ...
        for sv, p in obs["if_code_m"].items():
            ...
            if not candidates:
                continue
            record = min(candidates, key=lambda r: abs(t - (r.toc_gps - DAY).total_seconds()))
            if abs(t - (record.toc_gps - DAY).total_seconds()) <= 7200:
                _, elev = reference_model(record, p, t, station, 0.0)
                if elev >= 10:
                    records[sv] = record
        svs = sorted(records)
        if len(svs) < 4:
        ...
        clock = 0.0
        for _ in range(4):
            estimates = np.array([obs["if_code_m"][sv] - reference_model(records[sv], obs["if_code_m"][sv], t, station, clock)[0] for sv in svs])
        ...
        def residual(v):
            return np.array([reference_model(records[sv], obs["if_code_m"][sv], t, station + v[:3], v[3])[0] + v[3] - obs["if_code_m"][sv] for sv in svs])
        spp = least_squares(residual, np.array([0., 0., 0., clock]), diff_step=1e-4, xtol=1e-11, ftol=1e-11, gtol=1e-8)
    ...
    return {"epochs": epochs, "failures": failures, "status": "CALIBRATION_QUALIFIED" if not failures and len(epochs) == 11 else "CALIBRATION_NOT_QUALIFIED"}
