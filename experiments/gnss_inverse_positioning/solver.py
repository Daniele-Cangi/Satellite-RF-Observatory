"""Five-receiver emitted-event multilateration: no target-state input surface."""
...
def interpolate_event(codes, clocks, stations, u0=None, degree=3, time_offsets=None):
    codes, clocks, stations = map(np.asarray, (codes, clocks, stations))
    times = np.arange(-150., 151., 30.)
...
def algebraic_seeds(z, stations):
    ...
    if np.linalg.matrix_rank(a) != 4:
        raise ValueError("five-root algebraic matrix is rank deficient")
...
def uncertainty_box(z, stations, covariance, best, bias_m=20.):
    displacements = []
    max_semiaxis = float(np.sqrt(CHI95 * np.linalg.eigvalsh(best["covariance"][:3, :3])[-1]))
    for signs in product((-1., 1.), repeat=len(z)):
    ...
    return {"corners": displacements, "profile_axes": profiles, "statistical_95_radius_m": max_semiaxis, "bias_displacement_max_m": float(bias_radius), "total_95_outer_radius_m": float(max_semiaxis + bias_radius)}
