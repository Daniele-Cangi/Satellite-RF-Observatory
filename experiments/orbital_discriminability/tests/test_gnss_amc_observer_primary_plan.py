...
def test_exact_closed_parents_and_primary_boundary_are_bound() -> None:
    ...
    assert value["outcome"] == "AMC_OBSERVER_PRIMARY_PLAN_FROZEN"
...
def test_primary_role_window_and_partition_are_exact() -> None:
    ...
    assert value["observer"]["station"] == "AMC400USA"
    assert value["primary_artifact"]["logical_product"] == (
        "AMC400USA_R_20262210000_01D_30S_MO.crx.gz"
    )
    assert "/2026/221/" in value["primary_artifact"]["authority_url"]
    assert value["primary_artifact"]["preexisting_directory_description"] == {
        "bytes": 3_415_979,
        "last_modified": "2026-08-10 03:01:38",
        "directory_response_sha256": (
    ...
    assert value["partition"] == {
        "time_system": "GPS",
        "raw_start": "2026-08-09T05:41:30 GPS",
        "raw_stop": "2026-08-09T06:50:30 GPS",
        "raw_epochs": 139,
        "cadence_s": 30.0,
        "anchor_index": 0,
        ...
        "heldout_start": "2026-08-09T06:21:00 GPS",
        "heldout_epochs": 60,
...
def test_coordinate_has_one_fixed_anchor_and_zero_fitted_nuisance() -> None:
    ...
    assert coordinate["station_satellite_order"] == "AMC_G22_MINUS_AMC_G30"
...
def test_observer_independence_scope_is_explicit_and_not_overclaimed() -> None:
    value = compiled()
    observer = value["observer"]
    claim = value["claim_scope"]

    assert observer["receiver_serial"] == "3013929"
    assert observer["receiver_version"] == "5.6.0"
    assert observer["antenna"] == "TPSCR.G5C NONE"
    assert observer["antenna_serial"] == "1364-10065"
    assert observer["shared_receiver_family_with_pie"] is True
    assert claim["shared_receiver_family_limits_hardware_diversity"] is True
    assert claim["maximum_positive_claim"] == (
        "INDEPENDENT_OBSERVER_AND_PASS_REPLICATION_FOR_THIS_ORBIT_SIGNAL_FAMILY"
    )
    assert claim["not_generalized_beyond_amc_doy221"] is True
...
def test_revised_margin_regression_remains_strictly_positive() -> None:
    ...
    assert envelope["one_model_envelope_m"] == pytest.approx(3_669.850617323699)
    assert envelope["pairwise_decision_guard_m"] == pytest.approx(7_339.701234647398)
    assert envelope["remaining_margin_m"] == pytest.approx(154_907.49169172862)
...
def test_outcomes_separate_invalid_not_detectable_and_physical_scores() -> None:
    ...
    assert "AMC_HELD_OUT_ORBITAL_MODEL_PREFERRED" in outcomes
...
def test_parent_tampering_is_refused_before_plan_compilation(monkeypatch) -> None:
    ...
    with pytest.raises(plan.AmcPlanError, match="FROZEN_PARENT_CHANGED"):
        plan.plan(ROOT)


def test_cli_prints_only_public_minimal_summary(capsys) -> None:
    assert plan.main([]) == 0
...
def test_frozen_receipt_binds_source_manifest_plan_and_zero_access() -> None:
