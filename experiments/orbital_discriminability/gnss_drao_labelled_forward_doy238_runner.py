"""One-use runner for the frozen DRAO DOY238 labelled-forward experiment.

The scientific contract lives in ``gnss_drao_labelled_forward_doy238_executor``.
This source-specific runner adds only exact-locator materialization, a hash
receipt before decoding, typed terminal failures and volatile-buffer cleanup.
It is not a downloader, adapter or reusable GNSS execution framework.
"""
...
from experiments.orbital_discriminability import (
    gnss_drao_labelled_forward_doy238_executor as experiment,
)


RUNNER_VERSION: Final = "drao-labelled-forward-doy238-one-use-runner-v1"
SELECTION_NAME: Final = experiment.SELECTION_RECEIPT_NAME
SELECTION_SHA256: Final = "b79000671c21e155309caa06bc0a74819e49763871ddfaf323ce301fabe7e9d5"
EXECUTOR_SEAL_NAME: Final = "GNSS_DRAO_LABELLED_FORWARD_DOY238_EXECUTOR_MANIFEST.json"
EXECUTOR_SEAL_SHA256: Final = "6767b405a5e8968495c580c069975e0b643da10d3ab81045a1b93b48cdf44f62"
RUNNER_SEAL_NAME: Final = "GNSS_DRAO_LABELLED_FORWARD_DOY238_RUNNER_SEAL.json"
AUTHORITY_NAME: Final = "GNSS_DRAO_LABELLED_FORWARD_DOY238_AUTHORITY_CONSUMED.json"
MATERIALIZATION_NAME: Final = (
    "GNSS_DRAO_LABELLED_FORWARD_DOY238_MATERIALIZATION.json"
)
OUTCOME_NAME: Final = "GNSS_DRAO_LABELLED_FORWARD_DOY238_OUTCOME.json"
AUTHORITY_TOKEN: Final = "AUTHORIZE_DRAO_LABELLED_FORWARD_DOY238_ONCE"

ARTIFACT_NAME: Final = "DRAO00CAN_R_20262380000_01D_30S_MO.crx.gz"
ARTIFACT_URL: Final = (
    "https://igs.bkg.bund.de/root_ftp/IGS/obs/2026/238/" + ARTIFACT_NAME
)
ARTIFACT_BYTES: Final = 2_852_212
ARTIFACT_ETAG: Final = '"2b8574-659fd671597b7"'
ARTIFACT_LAST_MODIFIED: Final = "Thu, 27 Aug 2026 01:39:11 GMT"
...
class ArtifactMaterializationFailed(RuntimeError):
    """The exact selected artifact did not reach a complete-file hash."""
...
def verify_selection(root: Path) -> dict[str, object]:
    ...
    if canonical_sha256(path) != SELECTION_SHA256:
        raise RunnerDescriptionError("DRAO_DOY238_SELECTION_CHANGED")
    selection = _read_json(path)
    artifact = selection.get("artifact", {})
    http = selection.get("http_head_receipt", {})
    if (
        selection.get("state") != "DRAO_DOY238_ARTIFACT_SELECTED_UNOPENED"
        ...
        raise RunnerDescriptionError("DRAO_DOY238_SELECTION_STATE_CHANGED")
    return selection
...
def runner_manifest(root: Path) -> dict[str, object]:
    ...
    if canonical_sha256(Path(root) / EXECUTOR_SEAL_NAME) != EXECUTOR_SEAL_SHA256:
        raise RunnerDescriptionError("DRAO_DOY238_EXECUTOR_SEAL_CHANGED")
    experiment.verify_frozen_inputs(root)
    value = {
        "schema": "gnss-drao-labelled-forward-doy238-runner-manifest-v1",
        "version": RUNNER_VERSION,
        "state": "DRAO_DOY238_RUNNER_FROZEN_UNEXECUTED",
        "physical_question": (
            "DO_THE_SIX_FROZEN_RECEIVER_LABELLED_PATHS_PREDICT_THE_HELDOUT_"
            "SUFFIX_BETTER_THAN_THE_TWO_FROZEN_NULLS"
        ),
        "new_information": "ONE_TERMINAL_REAL_DRAO_DOY238_ORBITAL_VERSUS_NULL_EVENT",
        "selection": {
            "name": SELECTION_NAME,
        ...
        "frozen_executor": {
            ...
            "plan_raw_sha256": experiment.PLAN_RAW_SHA256,
            "prediction_bundle_raw_sha256": experiment.BUNDLE_RAW_SHA256,
        },
        "execution_order": [
...
def validate_seal(root: Path, seal_path: Path, expected_sha256: str) -> dict[str, object]:
    if len(expected_sha256) != 64 or canonical_sha256(seal_path) != expected_sha256:
        raise RunnerDescriptionError("DRAO_DOY238_RUNNER_SEAL_CHANGED")
    seal = _read_json(seal_path)
    if seal.get("state") != "DRAO_DOY238_RUNNER_FROZEN_UNEXECUTED":
        raise RunnerDescriptionError("DRAO_DOY238_RUNNER_SEAL_STATE_CHANGED")
    if seal.get("runner_source_canonical_sha256") != source_sha256():
        raise RunnerDescriptionError("DRAO_DOY238_RUNNER_SOURCE_CHANGED")
    if seal.get("runner_manifest_sha256") != manifest_sha256(root):
        raise RunnerDescriptionError("DRAO_DOY238_RUNNER_MANIFEST_CHANGED")
    if seal.get("selection_canonical_sha256") != SELECTION_SHA256:
        raise RunnerDescriptionError("DRAO_DOY238_RUNNER_SELECTION_CHANGED")
    if seal.get("live_authority_granted") is not False:
        raise RunnerDescriptionError("DRAO_DOY238_SEAL_GRANTED_LIVE_AUTHORITY")
    if any(seal.get("access_at_seal", {}).values()):
        raise RunnerDescriptionError("DRAO_DOY238_SEAL_USED_OBSERVATION")
    return seal
...
def _validate_response_identity(response: object) -> None:
    if int(response.status) != 200 or response.geturl() != ARTIFACT_URL:
        raise RunnerDescriptionError("DRAO_DOY238_ARTIFACT_LOCATOR_CHANGED")
    expected = {
    ...
    if actual != expected:
        raise RunnerDescriptionError("DRAO_DOY238_HTTP_IDENTITY_CHANGED")
...
def materialize() -> tuple[bytearray, dict[str, object]]:
    ...
    for attempt in range(1, MAX_TRANSPORT_ATTEMPTS + 1):
        ...
        try:
            request = Request(
                ...
                headers={
                    ...
                    "User-Agent": "Satellite-RF-Observatory/DRAO-DOY238-primary",
                },
                method="GET",
            )
            with urlopen(request, timeout=HTTP_TIMEOUT_S) as response:
                ...
                while block := response.read(64 * 1024):
                    ...
                    if len(payload) > MAX_COMPRESSED_BYTES:
                        raise RunnerDescriptionError("DRAO_DOY238_COMPRESSED_SIZE_LIMIT")
        ...
        if len(payload) != ARTIFACT_BYTES:
            ...
            continue
        receipt = {
            "schema": "gnss-drao-labelled-forward-doy238-materialization-v1",
            "state": "DRAO_DOY238_COMPLETE_HASH_RECORDED_BEFORE_DECODE",
    ...
    receipt = {
        "schema": "gnss-drao-labelled-forward-doy238-materialization-failure-v1",
    ...
    raise ArtifactMaterializationFailed(receipt)
...
def _not_evaluated(reason_class: str, reason: str) -> dict[str, object]:
    return {
        "schema": "gnss-drao-labelled-forward-doy238-outcome-v1",
        "outcome": "PRIMARY_NOT_EVALUATED",
        "reason_class": reason_class,
        "reason": reason,
        "clauses": {
            "artifact_complete_hash": "NOT_EVALUATED",
            "composite_identity": "NOT_EVALUATED",
            "typed_transform_ledger": "NOT_EVALUATED",
            "complete_six_prn_topology": "NOT_EVALUATED",
            "model_blind_witnesses": "NOT_EVALUATED",
            "orbital_versus_null_score": "NOT_EVALUATED",
        },
...
def run_once(
    ...
    if authority_token != AUTHORITY_TOKEN:
        raise PermissionError("DRAO_DOY238_ONE_USE_AUTHORITY_REQUIRED")
    ...
    if authority_path.exists() or outcome_path.exists():
        raise PermissionError("DRAO_DOY238_ONE_USE_AUTHORITY_ALREADY_CONSUMED")
    root = Path(__file__).resolve().parent
    seal = validate_seal(root, seal_path, expected_seal_sha256)
    authority = {
        "schema": "gnss-drao-labelled-forward-doy238-authority-consumed-v1",
        "state": "DRAO_DOY238_ONE_USE_AUTHORITY_CONSUMED_BEFORE_NETWORK",
    ...
    try:
        ...
        scan = experiment.scan_decoded_fixture(
            decoded, archive_site_id=experiment.frozen.STATION
        )
        admission, coordinate = experiment.admit_model_blind(scan)
        score = experiment.score_admitted_coordinate(coordinate, root)
        outcome = {
            "schema": "gnss-drao-labelled-forward-doy238-outcome-v1",
    ...
    except experiment.ForwardMeasurementInvalid as exc:
        outcome = {
            "schema": "gnss-drao-labelled-forward-doy238-outcome-v1",
    ...
    except (RunnerDescriptionError, experiment.ForwardDescriptionError) as exc:
...
def main() -> None:
    ...
    if not args.execute_live or args.seal is None:
        raise SystemExit("DRAO_DOY238_FROZEN_RUNNER_AND_ONE_USE_AUTHORITY_REQUIRED")
    print(
        strict_json(
            run_once(
