"""Sealed one-shot executor for the prospective AMC DOY221 observation.
...
from experiments.orbital_discriminability import (
    gnss_amc_observer_predictions as prediction,
)
from experiments.orbital_discriminability import (
    gnss_amc_observer_primary_plan as plan,
)
from experiments.orbital_discriminability import (
    gnss_amc_observer_qualification as qualified,
)


EXECUTOR_VERSION: Final = "amc-g22-g30-doy221-one-shot-executor-v1"
EXECUTOR_SEAL_NAME: Final = "AMC_OBSERVER_PRIMARY_EXECUTOR_SEAL.json"
OUTCOME_NAME: Final = "AMC_OBSERVER_PRIMARY_OUTCOME.json"
AUTHORITY_MARKER_NAME: Final = "AMC_OBSERVER_PRIMARY_AUTHORITY_CONSUMED.json"
AUTHORITY_TOKEN: Final = "AUTHORIZE_AMC_DOY221_PRIMARY_ONCE"
...
PREFERRED_OUTCOMES: Final = {
    "ORBITAL_G22": "AMC_HELD_OUT_ORBITAL_MODEL_PREFERRED",
...
PRIMARY_DIRECTORY_COMPONENTS: Final = ("gnss", "data", "daily", "2026", "221")
...
class AmcExecutorError(ValueError):
...
def _read_strict_json(path: Path) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise AmcExecutorError(f"FROZEN_ARTIFACT_NOT_OBJECT:{Path(path).name}")
    return value
...
def _require_hash(root: Path, name: str, digest: str) -> Path:
    ...
    if not path.is_file() or canonical_sha256(path) != digest:
        raise AmcExecutorError(f"FROZEN_ARTIFACT_CHANGED:{name}")
    return path
...
def _expected_header_transform(root: Path) -> dict[str, object]:
    ...
    if not isinstance(header, Mapping):
        raise AmcExecutorError("QUALIFIED_HEADER_MISSING")
    if summary.get("full_joint_window") is not True:
        raise AmcExecutorError("QUALIFIED_WINDOW_CHANGED")
    result = {
    ...
    if result["required_phase_shift_records"] != ["G L1C", "G L2W"]:
        raise AmcExecutorError("QUALIFIED_PHASE_TRANSFORM_CHANGED")
    if result["applied_bias_records"] or result["receiver_clock_offset_applied"] != 0:
        raise AmcExecutorError("QUALIFIED_RECEIVER_TRANSFORM_CHANGED")
    strict_json(result)
    return result
...
def validate_frozen_inputs(
    ...
    if seal.get("state") != "AMC_OBSERVER_PRIMARY_PREDICTION_FROZEN":
        raise AmcExecutorError("PREDICTION_SEAL_STATE_CHANGED")
    if seal.get("predictions", {}).get("canonical_sha256") != PREDICTIONS_SHA256:
        raise AmcExecutorError("PREDICTION_SEAL_BINDING_CHANGED")
    ...
    if not isinstance(authority, Mapping) or any(
        ...
        raise AmcExecutorError("PREDICTION_SEAL_GRANTED_ACCESS")
    ...
    if (
        ...
        raise AmcExecutorError("PRIMARY_BOUNDARY_CHANGED")
...
def build_executor_seal(root: Path) -> dict[str, object]:
    ...
    try:
        result = {
            "schema": "amc-observer-primary-executor-seal-v1",
            "state": "AMC_OBSERVER_PRIMARY_EXECUTOR_FROZEN_UNOPENED",
...
def validate_executor_seal(
    ...
    if len(expected_sha256) != 64 or canonical_sha256(seal_path) != expected_sha256:
        raise AmcExecutorError("EXECUTOR_SEAL_SHA256_CHANGED")
    seal = _read_strict_json(seal_path)
    if seal.get("state") != "AMC_OBSERVER_PRIMARY_EXECUTOR_FROZEN_UNOPENED":
        raise AmcExecutorError("EXECUTOR_SEAL_STATE_CHANGED")
    if seal.get("source_sha256") != source_sha256():
        raise AmcExecutorError("EXECUTOR_SOURCE_CHANGED")
    if seal.get("manifest_sha256") != manifest_sha256(root):
        raise AmcExecutorError("EXECUTOR_MANIFEST_CHANGED")
    if seal.get("dependencies") != dependency_versions():
        raise AmcExecutorError("EXECUTOR_DEPENDENCIES_CHANGED")
    if seal.get("authority", {}).get("live_execution_authorized_by_seal") is not False:
        raise AmcExecutorError("EXECUTOR_SEAL_GRANTED_LIVE_AUTHORITY")
    if any(seal.get("access_at_seal", {}).values()):
        raise AmcExecutorError("EXECUTOR_SEAL_USED_PRIMARY")
    curves, transform = validate_frozen_inputs(root)
    ...
    if seal.get("qualified_header_transform_sha256") != expected_transform_hash:
        for curve in curves.values():
            curve.fill(0.0)
        raise AmcExecutorError("QUALIFIED_HEADER_TRANSFORM_CHANGED")
...
def _new_gssc_session() -> Any:
    ...
    try:
        ...
        response = session.post(
            ...
            headers={"User-Agent": "Satellite-RF-Observatory/AMC-primary"},
...
def score_coordinate(
    ...
    if set(normalized) != set(HYPOTHESES) or any(
        ...
        raise AmcExecutorError("FROZEN_HYPOTHESIS_CURVES_INVALID")
    scores = []
...
def run_once(
    ...
    *,
    ...
    if authority_token != AUTHORITY_TOKEN:
        raise PermissionError("AMC_DOY221_PRIMARY_AUTHORITY_REQUIRED")
    ...
    if outcome_path.exists() or marker_path.exists():
        raise PermissionError("AMC_DOY221_PRIMARY_AUTHORITY_ALREADY_CONSUMED")
    root = Path(__file__).resolve().parent
    seal, curves, expected_transform = validate_executor_seal(
        root, executor_seal_path, expected_seal_sha256
    )
    marker = {
        "schema": "amc-observer-primary-authority-consumed-v1",
    ...
    try:
        ...
        if not artifact.get("complete_file_sha256"):
            raise AmcExecutorError("COMPLETE_HASH_REQUIRED_BEFORE_DECODE")
        ...
        outcome = {
            "schema": "amc-observer-primary-outcome-v1",
            "executor_version": EXECUTOR_VERSION,
            ...
            "claim_scope": (
                ...
                if score["outcome"] == "AMC_HELD_OUT_ORBITAL_MODEL_PREFERRED"
                else "NO_POSITIVE_AMC_ORBITAL_CLAIM"
            ),
        }
...
def main() -> None:
    ...
    if args.write_executor_seal:
        ...
        print(
            strict_json(
                {
                    "outcome": "AMC_OBSERVER_PRIMARY_EXECUTOR_FROZEN_UNOPENED",
