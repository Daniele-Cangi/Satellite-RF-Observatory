"""Sealed one-shot ALGO/MDO DOY223 primary executor.

Importing this experiment-specific module performs no network request, header
parsing, or observation decoding.  Live access requires both a separately
supplied authority token and the exact hash of a post-commit executor seal.
"""
...
from experiments.orbital_discriminability import gnss_independent_pair_doy223_predictions as prediction
from experiments.orbital_discriminability import gnss_independent_pair_doy223_primary_plan as frozen
...
EXECUTOR_VERSION: Final = "g22-g30-algo-mdo-doy223-primary-executor-v1"
EXECUTOR_SEAL_NAME: Final = "GNSS_INDEPENDENT_PAIR_DOY223_EXECUTOR_SEAL.json"
OUTCOME_NAME: Final = "GNSS_INDEPENDENT_PAIR_DOY223_OUTCOME.json"
AUTHORITY_TOKEN: Final = "AUTHORIZE_ALGO_MDO_DOY223_PRIMARY_ONCE"

PLAN_DOCUMENT_NAME: Final = "GNSS_INDEPENDENT_PAIR_DOY223_PRIMARY_PLAN.md"
PLAN_DOCUMENT_SHA256: Final = (
    "8c6890ba6a9638739f9402d4c5ea3264d8f74aef82b8c83b58902ac5cd9e1291"
)
...
ONE_MODEL_CALIBRATION_ENVELOPE_M: Final = (
    frozen.SCREEN_PAIRWISE_GUARD_M / 2.0
)
...
MAX_ATTEMPTS_PER_MIRROR: Final = 2
MAX_TOTAL_ATTEMPTS_PER_PRODUCT: Final = 4
MAX_WALL_CLOCK_S_PER_PRODUCT: Final = 900.0
CONNECT_TIMEOUT_S: Final = 30.0
IDLE_TIMEOUT_S: Final = 180.0
BLOCK_BYTES: Final = 1024 * 1024
MAX_COMPRESSED_BYTES: Final = primary.MAX_COMPRESSED_BYTES
...
PRODUCTS: Final = tuple(
    ProductPlan(item.station, item.name, item.mirrors)
    for item in frozen.PRODUCTS
)


class Doy223ExecutorError(RuntimeError):
    """The executor, frozen evidence, or seal binding changed."""


class Doy223MaterializationError(RuntimeError):
    """No complete artifact was produced inside the frozen transport budget."""
...
Materializer = Callable[[ProductPlan], tuple[bytearray, dict[str, object]]]
...
def canonical_sha256(path: Path) -> str:
    return sha256(
        Path(path).read_bytes().replace(bytes((13, 10)), bytes((10,)))
    ).hexdigest()
...
def expected_raw_gps_epochs() -> tuple[datetime, ...]:
    ...
    if structural.format_gps_epoch(epochs[-1]) != (
        ...
        raise Doy223ExecutorError("PRIMARY_GRID_CHANGED")
    return epochs


def _locator(product: ProductPlan, mirror: str) -> primary.ProductLocator:
    return primary.ProductLocator(product.station, product.name, mirror)


def validate_kernel_contract() -> None:
    bindings = {
        "PRIMARY_MODEL_BLIND_KERNEL": (
            primary.__file__,
            PRIMARY_KERNEL_SHA256,
        ),
        "RINEX_STRUCTURAL_PARSER": (
            rinex.__file__,
            RINEX_PARSER_SHA256,
        ),
    ...
    for name, (path, expected) in bindings.items():
        if canonical_sha256(Path(path)) != expected:
            raise Doy223ExecutorError(f"{name}_CHANGED")
    if (
        ...
        or primary.frozen.CALIBRATION_EPOCHS
        ...
        raise Doy223ExecutorError("REUSED_KERNEL_SHAPE_CHANGED")
    if primary.frozen.plan()["measurement_coordinate"][
        ...
        raise Doy223ExecutorError("REUSED_COORDINATE_CHANGED")
    if ONE_MODEL_CALIBRATION_ENVELOPE_M != 1_571.0820742800613:
        raise Doy223ExecutorError("ONE_MODEL_ENVELOPE_CHANGED")


def executor_manifest() -> dict[str, object]:
    validate_kernel_contract()
    transport = frozen.plan()["transport_materialization"]
    result = {
        "schema": "gnss-independent-pair-doy223-executor-manifest-v1",
        ...
        "transport": {
            **transport,
            "partial_bytes": "EPHEMERAL_RAM_QUARANTINE",
            "stable_resume_validators": ["ETAG", "LAST_MODIFIED"],
            "complete_hashes_before_first_decode": 2,
        },
        "persistence": {
            "compressed_rinex": 0,
            "decoded_rinex": 0,
        ...
        "forbidden": [
            ...
            "CROSS_MIRROR_PARTIAL_APPEND",
            "RETRY_AFTER_COMPLETE_HASH_OR_DECODE",
...
def _load_qualified_headers(
    ...
    if not isinstance(rows, list):
        raise Doy223ExecutorError("QUALIFICATION_HEADERS_INVALID")
    result = {
        str(row["station"]): row for row in rows if isinstance(row, Mapping)
    }
    if list(result) != ["ALGO00CAN", "MDO100USA"]:
        raise Doy223ExecutorError("QUALIFICATION_HEADER_ORDER_CHANGED")
...
def validate_frozen_inputs(
    ...
    if canonical_sha256(base / PLAN_DOCUMENT_NAME) != PLAN_DOCUMENT_SHA256:
        raise Doy223ExecutorError("PRIMARY_PLAN_DOCUMENT_CHANGED")
    ...
    if canonical_sha256(prediction_path) != PREDICTIONS_SHA256:
        raise Doy223ExecutorError("PRIMARY_PREDICTIONS_CHANGED")
    if canonical_sha256(prediction_seal_path) != PREDICTION_SEAL_SHA256:
        raise Doy223ExecutorError("PRIMARY_PREDICTION_SEAL_CHANGED")
    value = json.loads(
    ...
    if seal.get("state") != "PRIMARY_PLAN_AND_PREDICTION_FROZEN":
        raise Doy223ExecutorError("PRIMARY_PREDICTION_NOT_FROZEN")
    if seal.get("plan_manifest_sha256") != frozen.manifest_sha256():
        raise Doy223ExecutorError("PRIMARY_PLAN_BINDING_CHANGED")
    if seal.get("authority", {}).get("primary_access_authorized_by_seal"):
        raise Doy223ExecutorError("PREDICTION_SEAL_AUTHORITY_CHANGED")
    access = seal.get("access_at_seal", {})
    if not isinstance(access, Mapping) or any(
        int(access.get(field, -1)) != 0
        for field in (
            "observation_locator_requests",
        ...
        raise Doy223ExecutorError("PREDICTION_SEAL_USED_OBSERVATIONS")
    return curves, _load_qualified_headers(base)


def _stable_validator(headers_value: Mapping[str, str]) -> tuple[str, str] | None:
    etag = headers_value.get("ETag") or headers_value.get("etag")
    if etag:
        return "ETAG", str(etag)
    modified = headers_value.get("Last-Modified") or headers_value.get(
        "last-modified"
    )
    if modified:
        return "LAST_MODIFIED", str(modified)
    return None


def _open(request: Request, timeout: float) -> Response:
    response = urlopen(request, timeout=timeout)
    socket_value = getattr(
    ...
    if socket_value is None:
        response.close()
        raise Doy223ExecutorError("HTTP_IDLE_TIMEOUT_BINDING_UNAVAILABLE")
    socket_value.settimeout(IDLE_TIMEOUT_S)
...
def _attempt_download(
    product: ProductPlan,
    mirror: str,
    state: PartialDownload,
    *,
    opener: OpenRequest,
) -> tuple[bool, int | None]:
    offset = len(state.payload) if state.validator is not None else 0
    if offset == 0 and state.payload:
        state.payload[:] = b"\x00" * len(state.payload)
        state.payload.clear()
    request_headers = {
        "User-Agent": "Satellite-RF-Observatory/gnss-doy223-primary"
    }
    if offset:
        request_headers["Range"] = f"bytes={offset}-"
        request_headers["If-Range"] = state.validator[1]
    request = Request(mirror, headers=request_headers)
    with opener(request, CONNECT_TIMEOUT_S) as response:
        status = int(getattr(response, "status", 200))
        response_validator = _stable_validator(response.headers)
        if offset:
            content_range = str(response.headers.get("Content-Range", ""))
            if (
                status != 206
                or not content_range.startswith(f"bytes {offset}-")
                or response_validator != state.validator
            ):
                raise Doy223ExecutorError(
                    f"RESUME_VALIDATION_FAILED:{product.station}"
                )
        elif status not in {200, 206}:
            raise Doy223ExecutorError(
                f"UNEXPECTED_HTTP_STATUS:{product.station}:{status}"
            )
        if state.validator is None:
            state.validator = response_validator
        content_length_value = response.headers.get("Content-Length")
        expected_total = None
        if content_length_value is not None:
            expected_total = offset + int(content_length_value)
        while True:
            block = response.read(BLOCK_BYTES)
            if not block:
                break
            state.payload.extend(block)
            if len(state.payload) > MAX_COMPRESSED_BYTES:
                raise Doy223ExecutorError(
                    f"COMPRESSED_SIZE_LIMIT:{product.station}"
                )
        if not state.payload:
            raise Doy223ExecutorError(f"EMPTY_ARTIFACT:{product.station}")
        if expected_total is not None and len(state.payload) != expected_total:
            raise OSError(
                f"INCOMPLETE_HTTP_BODY:{product.station}:"
                f"{len(state.payload)}:{expected_total}"
            )
        return bool(offset), expected_total


def materialize_bounded(
    product: ProductPlan,
    *,
    opener: OpenRequest = _open,
    monotonic: Callable[[], float] = time.monotonic,
) -> tuple[bytearray, dict[str, object]]:
    started = monotonic()
    total_attempts = 0
    failures: list[dict[str, object]] = []
    for mirror_index, mirror in enumerate(product.mirrors):
        state = PartialDownload(bytearray())
        for mirror_attempt in range(1, MAX_ATTEMPTS_PER_MIRROR + 1):
            if total_attempts >= MAX_TOTAL_ATTEMPTS_PER_PRODUCT:
                break
            if monotonic() - started > MAX_WALL_CLOCK_S_PER_PRODUCT:
                break
            total_attempts += 1
            before = len(state.payload)
            try:
                resumed, _ = _attempt_download(
                    product,
                    mirror,
                    state,
                    opener=opener,
                ...
                receipt = {
                    ...
                    "attempt_on_selected_mirror": mirror_attempt,
                    "resume_used": resumed,
                    "stable_validator": (
                        {
                            "kind": state.validator[0],
                            "value": state.validator[1],
                        }
                        if state.validator
                        else None
                    ),
                    "complete_file_bytes": len(state.payload),
                    "complete_file_sha256": sha256(state.payload).hexdigest(),
                    "hash_before_any_decode": True,
                    "cross_mirror_partial_append": False,
                }
                strict_json(receipt)
                return state.payload, receipt
            except (HTTPError, URLError, TimeoutError, OSError) as exc:
                failures.append(
                    {
                        "mirror_index": mirror_index,
                        "attempt": mirror_attempt,
                        "partial_bytes_before": before,
                        "partial_bytes_after": len(state.payload),
                ...
                if state.validator is None:
                    state.payload[:] = b"\x00" * len(state.payload)
                    state.payload.clear()
            except Doy223ExecutorError as exc:
                failures.append(
                    {
                        "mirror_index": mirror_index,
                        "attempt": mirror_attempt,
    ...
    receipt = {
        ...
        "attempts_total": total_attempts,
        "maximum_attempts": MAX_TOTAL_ATTEMPTS_PER_PRODUCT,
        "wall_clock_budget_s": MAX_WALL_CLOCK_S_PER_PRODUCT,
        "mirror_count": len(product.mirrors),
        "failures": failures,
        "complete_file_bytes": None,
        "complete_file_sha256": None,
    ...
    strict_json(receipt)
    raise Doy223MaterializationError(
        f"ARTIFACT_MATERIALIZATION_FAILED:{product.station}", receipt
    )
...
def _validate_header(
    ...
    if str(parsed["marker_name"]) != station[:4]:
        raise primary.PrimaryMeasurementInvalid(
            f"MARKER_IDENTITY_MISMATCH:{station}"
        )
    ...
    if first > epochs[0] or last < epochs[-1]:
        raise primary.PrimaryMeasurementInvalid(
            f"FROZEN_WINDOW_NOT_COVERED:{station}"
        )
    ...
    if missing:
        raise primary.PrimaryMeasurementInvalid(
            f"REQUIRED_SIGNAL_FAMILY_NOT_DECLARED:{station}:"
            f"{','.join(missing)}"
        )
    transform_checks = {
    ...
    return {
        ...
        "receiver_clock_offset_applied": parsed[
...
def _read_window_records(
    ...
    dict[tuple[datetime, str], rinex._Record],
    dict[datetime, int],
]:
    epochs = expected_raw_gps_epochs()
    epoch_set = set(epochs)
    start, stop = epochs[0], epochs[-1]
    records: dict[tuple[datetime, str], rinex._Record] = {}
    ...
    while True:
        ...
        if flag not in {0, 1}:
            raise primary.PrimaryMeasurementInvalid(
                f"RECORD_INVALID:EPOCH_FLAG_{flag}"
            )
        for _ in range(satellite_count):
            try:
...
def scan_decoded(
    ...
    try:
        ...
        raise primary.PrimaryMeasurementInvalid(
            f"HEADER_INVALID:{product.station}:{exc}"
        ) from exc
    except Exception as exc:
        raise Doy223ExecutorError(
            f"HEADER_DESCRIPTION_ERROR:{product.station}:"
            f"{type(exc).__name__}:{exc}"
        ) from exc
    header = _validate_header(parsed, product, expected_header)
    system_types = {
        system: tuple(values)
    ...
    phase_cycles = np.full(
        (frozen.RAW_EPOCHS, 2, 2), np.nan, dtype=np.float64
    )
    ...
    for row, epoch in enumerate(expected_raw_gps_epochs()):
        if flags.get(epoch) != 0:
            raise primary.PrimaryMeasurementInvalid(
                f"EPOCH_ABSENT_OR_FLAGGED:{product.station}:"
        ...
        for satellite_index, satellite in enumerate(SATELLITES):
            ...
            if record is None:
                raise primary.PrimaryMeasurementInvalid(
                    f"SATELLITE_RECORD_ABSENT:{product.station}:"
            ...
            for phase_index, observable in enumerate(CORE_PHASE):
                ...
                if field_index >= record.field_count:
                    ...
                    raise primary.PrimaryMeasurementInvalid(
                        f"TRAILING_FIELD_OMITTED:{product.station}:"
                        f"{structural.format_gps_epoch(epoch)}:"
                        f"{satellite}:{observable}"
                    )
                field = record.fields[field_index]
                if not field[:14].strip():
                    counts["BLANK"] += 1
                    raise primary.PrimaryMeasurementInvalid(
                        f"FIELD_BLANK:{product.station}:"
                        f"{structural.format_gps_epoch(epoch)}:"
                        f"{satellite}:{observable}"
                    )
                ...
                if lli != "ZERO_OR_BLANK":
                    raise primary.PrimaryMeasurementInvalid(
                        f"NONZERO_OR_INVALID_LLI:{product.station}:"
                        f"{structural.format_gps_epoch(epoch)}:"
                        f"{satellite}:{observable}:{lli}"
                    )
                try:
...
def measurement_coordinate(
    ...
    code_passed = True
    required = np.asarray(
        frozen.CODE_REQUIRED_RAW_INDICES, dtype=np.int64
    )
    for scan in scans:
        for satellite_index, satellite in enumerate(SATELLITES):
            for code_index, observable in enumerate(SAME_PATH_CODE):
    ...
    alpha, beta = frozen.plan()["coordinate"][
        "ionosphere_free_coefficients"
    ]
...
def score_coordinate(
    ...
    if observed.shape != (frozen.FEATURE_EPOCHS,) or not np.all(
    ...
    if set(normalized) != set(HYPOTHESES) or any(
        ...
        raise Doy223ExecutorError("FROZEN_HYPOTHESIS_CURVES_INVALID")
...
def build_executor_seal(root: Path) -> dict[str, object]:
    ...
    try:
        result = {
            "schema": "gnss-independent-pair-doy223-executor-seal-v1",
            ...
            "prediction_sha256": PREDICTIONS_SHA256,
            "prediction_seal_sha256": PREDICTION_SEAL_SHA256,
            "qualification_outcome_sha256": (
                frozen.QUALIFICATION_OUTCOME_SHA256
            ),
            "qualification_summary_sha256": QUALIFICATION_SUMMARY_SHA256,
...
def validate_executor_seal(
    ...
    if len(expected_seal_sha256) != 64 or (
        canonical_sha256(seal_path) != expected_seal_sha256
    ):
        raise Doy223ExecutorError("EXECUTOR_SEAL_SHA256_MISMATCH")
    seal = json.loads(
    ...
    for field, expected in checks.items():
        if seal.get(field) != expected:
            raise Doy223ExecutorError(f"EXECUTOR_SEAL_{field.upper()}_CHANGED")
    access = seal.get("access_at_seal", {})
    if not isinstance(access, Mapping) or any(
        int(access.get(field, -1)) != 0
        for field in (
        ...
        raise Doy223ExecutorError("EXECUTOR_SEAL_USED_OBSERVATIONS")
    ...
    if seal.get("qualified_header_transform_sha256") != sha256(
        ...
        for curve in curves.values():
            curve.fill(0.0)
        raise Doy223ExecutorError("QUALIFIED_HEADER_TRANSFORM_CHANGED")
...
def run_once(
    ...
    *,
    materializer: Materializer = materialize_bounded,
) -> dict[str, object]:
    if authority_token != AUTHORITY_TOKEN:
        raise PermissionError("ALGO_MDO_DOY223_PRIMARY_AUTHORITY_REQUIRED")
    ...
    try:
        for product in PRODUCTS:
            payload, artifact = materializer(product)
            compressed.append(payload)
            artifacts.append(artifact)
        if len(artifacts) != 2 or any(
            not row.get("complete_file_sha256") for row in artifacts
        ):
            raise Doy223ExecutorError(
                "BOTH_COMPLETE_HASHES_REQUIRED_BEFORE_DECODE"
            )
        for product, payload in zip(PRODUCTS, compressed, strict=True):
            rinex_payload = primary.decode_in_memory(payload, product.station)
            decoded.append(rinex_payload)
            scans.append(
                scan_decoded(
                    rinex_payload,
                    product,
                    expected_headers[product.station],
                )
            )
        ...
        try:
            ...
            coordinate.fill(0.0)
        outcome = {
            "schema": "gnss-independent-pair-doy223-outcome-v1",
            "executor_version": EXECUTOR_VERSION,
            ...
            "retry": {
                "transport_only_before_both_hashes": True,
                "attempts": [row["attempts_total"] for row in artifacts],
                "network_attempts_after_both_hashes": 0,
                "retry_after_decode": False,
                "endpoint_date_feature_or_threshold_substitution": False,
            },
            "claim_scope": (
    ...
    except Doy223MaterializationError as exc:
        materialization_failure = exc.receipt
        outcome = {
            "schema": "gnss-independent-pair-doy223-outcome-v1",
            "executor_version": EXECUTOR_VERSION,
            ...
            "failed_artifact": materialization_failure,
            "heldout_comparison": "NOT_EVALUATED",
        ...
        outcome = {
            "schema": "gnss-independent-pair-doy223-outcome-v1",
            "executor_version": EXECUTOR_VERSION,
            "execution_state": "PRIMARY_SOFTWARE_FAILURE",
            "physical_outcome": None,
...
def main() -> None:
    ...
    if not args.execute_live or args.executor_seal is None:
        raise SystemExit(
            "OFFLINE_EXECUTOR_FREEZE_OR_SEPARATE_LIVE_AUTHORITY_REQUIRED"
        )
    print(
        strict_json(
            run_once(
